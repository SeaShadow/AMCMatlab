...
%# ------------------------------------------------------------------------
if exist('statisticsArrayAnalysis_copy.dat', 'file') == 2
    results = csvread('statisticsArrayAnalysis_copy.dat');
    %# Remove zero rows
    results(all(results==0,2),:)=[];
    [mr,nr] = size(results);
else
    disp('-----------------------------------------------------------------');
    disp('File statisticsArrayAnalysis_copy.dat does not exist!');
    disp('-----------------------------------------------------------------');
    %break;
end
%# ------------------------------------------------------------------------
...
if msa > 1
    ...
    h = bar(x,y1,'r');
    if enablePlotTitle == 1
        title('{\bf Starboad Kiel Probe}','FontSize',setGeneralFontSize);
    end
...
%     %legend boxoff;
    ...
    h = bar(x,y2,'b');
    if enablePlotTitle == 1
        title('{\bf Port Kiel Probe}','FontSize',setGeneralFontSize);
    end
    xlabel('{\bf Run number [#]}','FontSize',setGeneralFontSize);
    ylabel('{\bf Standard Deviation [-]}','FontSize',setGeneralFontSize);
    grid on;
...
%     %legend boxoff;
    ...
    h = bar(x,y1,'r');
    if enablePlotTitle == 1
        title('{\bf Starbaord Kiel Probe}','FontSize',setGeneralFontSize);
    end
    xlabel('{\bf Run number [#]}','FontSize',setGeneralFontSize);
