%# ------------------------------------------------------------------------
%# Pitot Static Tube (PST) and Differential Pressure Transducer (DPT) Calibration
%# ------------------------------------------------------------------------
%#
...
% Time series plot
enableTimeSeriesPlot      = 0;    % Enable or disable time series plot
...
%# ************************************************************************
%# Plot 1: Speed vs. Voltage
%# ************************************************************************

% Remove zero rows from resultsArrayCalib
resultsArrayCalib = resultsArrayCalib(any(resultsArrayCalib,2),:);
M = resultsArrayCalib;
...
if enableCommandWindowOutput == 1
    ...
    EoFEqn = sprintf('EoF (Avg): y=%sx^4%sx^3%sx^2%sx%s | R^2: %s',p1,p2,p3,p4,p5,gofrs);
    disp(EoFEqn);
end
...
% Curve fitting
[fitobject2,gof2,output2] = fit(x2,y2,'poly4');
...
if enableCommandWindowOutput == 1
    ...
    p4   = sprintf(setDecimals4,cval(4));
    p5   = sprintf(setDecimals5,cval(5));
    gofrs = sprintf('%0.2f',gof2.rsquare);
    EoFEqn2 = sprintf('EoF (Min): y=%sx^4%sx^3%sx^2%sx%s | R^2: %s',p1,p2,p3,p4,p5,gofrs);
    disp(EoFEqn2);
end
...
% Curve fitting
[fitobject3,gof3,output3] = fit(x3,y3,'poly4');
...
if enableCommandWindowOutput == 1
    ...
    p4   = sprintf(setDecimals4,cval(4));
    p5   = sprintf(setDecimals5,cval(5));
    gofrs = sprintf('%0.2f',gof3.rsquare);
    EoFEqn3 = sprintf('EoF (Max): y=%sx^4%sx^3%sx^2%sx%s | R^2: %s',p1,p2,p3,p4,p5,gofrs);
    disp(EoFEqn3);
end
...
% Curve fitting
[fitobject4,gof4,output4] = fit(x4,y4,'poly4');
cvalues4                  = coeffvalues(fitobject4);

%# Plotting ---------------------------------------------------------------
h1 = plot(fitobject,'-',x1,y1,'*');
legendInfo{1}  = 'Avg CF(x-zero)';
legendInfo{2}  = 'Avg CF(x-zero) (curve fit)';
% Minimum value
hold on;
h2 = plot(fitobject2,'--');
legendInfo{3}  = 'Min CF(x-zero) (curve fit)';
set(h2(1),'Color',setColor{10},'Marker','none','LineStyle',setLineStyle1,'linewidth',setLineWidthThin);
% Maximum value
hold on;
h3 = plot(fitobject3,'-.');
legendInfo{4}  = 'Max CF(x-zero) (curve fit)';
set(h3(1),'Color',setColor{10},'Marker','none','LineStyle',setLineStyle2,'linewidth',setLineWidthThin);
% Raw values
h4 = plot(fitobject4,'-',x4,y4,'*');
legendInfo{5}  = 'Raw values (incl. zero)';
xlabel('{\bf DPT output (V)}','FontSize',setGeneralFontSize);
...
%# Line, colors and markers
set(h1(1),'Color',setColor{10},'Marker',setMarker{3},'MarkerSize',setMarkerSize,'LineWidth',setLineWidthMarker);
set(h1(2),'Color',setColor{10},'Marker','none','LineStyle',setLineStyle,'linewidth',setLineWidthThin);
set(h4(1),'Color',setColor{10},'Marker',setMarker{1},'MarkerSize',setMarkerSize,'LineWidth',setLineWidthMarker);
...
%# Annotations
text(0.15,0.42,EoFEqn2,'FontSize',setGeneralFontSize-1,'color','k','FontWeight','normal');
text(0.15,0.27,EoFEqn,'FontSize',setGeneralFontSize-1,'color','k','FontWeight','normal');
text(0.15,0.12,EoFEqn3,'FontSize',setGeneralFontSize-1,'color','k','FontWeight','normal');
